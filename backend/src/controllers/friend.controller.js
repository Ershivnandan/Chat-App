...
export const sendFriendRequest = async (req, res) => {
    ...
    try {
      ...
      const newRequest = new FriendRequest({ senderId, receiverId, senderName: sender.fullName });
      await newRequest.save();
  
      const receiverSocketId = getReceiverSocketId(receiverId);
      if (receiverSocketId) {
        io.to(receiverSocketId).emit("friendRequestReceived", { senderId, receiverId, senderName: sender.fullName });
      }
...
export const rejectFriendRequest = async (req, res) => {
    ...
    try {
      ...
      // Emit the friendRequestRejected event to notify the sender
      const senderSocketId = getReceiverSocketId(request.senderId);
      if (senderSocketId) {
        io.to(senderSocketId).emit("friendRequestRejected", { senderId: request.senderId, receiverId: request.receiverId });
      }
